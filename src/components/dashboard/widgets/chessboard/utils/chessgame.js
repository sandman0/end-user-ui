...
export default class ChessGame {
    constructor (fen = undefined, side = 'w') {
        this.game = Chess();
    ...
    getBoard () {
        const board = this.game.board(),
            boardArray = this.transformBoardToArray(board);
    ...
    calculateMove (sourceIndex, targetIndex) {
        const sourceStr = this.getPositionStringForIndex(sourceIndex),
            targetStr = this.getPositionStringForIndex(targetIndex);
    ...
    getAvailableMoves (index) {
        const moves = this.game.moves({
            ...
            moveIndeces = moves.map((move) => this.getIndexForPositionString(move.to));
    ...
    getPositionStringForIndex (index) {
        ...
        const number = Math.abs(row - 7) + 1,
            letter = String.fromCharCode(97 + column);
    ...
    getCoordinatesForPositionString (positionString = 'a1') {
        let column = positionString.toLowerCase().charCodeAt(0) - 97,
            row = Number.parseInt(positionString[1]) - 1;
